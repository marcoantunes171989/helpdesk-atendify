...
BEGIN
    -- Setup: Criar produto e cliente de teste (colunas corretas: descricao, estoque, valor_venda, valor_custo)
    INSERT INTO public.produtos (descricao, estoque, valor_venda, valor_custo)
    VALUES ('Produto Teste - Consistência', 100, 50, 20)
    RETURNING id, estoque INTO v_produto_id, v_estoque_inicial;

    INSERT INTO public.clientes (nome, email)
    VALUES ('Cliente Teste de Conversão', 'teste_conversao@example.com')
    RETURNING id INTO v_cliente_id;

    -- Passo 1: Criar posse (deve baixar 10 do estoque via trigger tr_posse_stock)
    ...
    -- Passo 2: Converter em venda (não deve baixar o estoque novamente devido à flag app.is_posse_conversion)

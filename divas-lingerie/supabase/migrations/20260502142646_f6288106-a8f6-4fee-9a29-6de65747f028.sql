...
BEGIN
    ...
    VALUES ('TST-003', 'Produto Teste - Consistência v3', 100, 50, 20)
    RETURNING id, estoque INTO v_produto_id, v_estoque_inicial;

    INSERT INTO public.clientes (nome)
    VALUES ('Cliente Teste de Conversão v3')
    ...
    -- Cleanup (Remover em ordem inversa de dependência)
    DELETE FROM public.auditoria_movimentacoes WHERE produto_id = v_produto_id;
    DELETE FROM public.auditoria_estoque WHERE produto_id = v_produto_id;
    DELETE FROM public.historico_posses WHERE posse_id = v_posse_id;

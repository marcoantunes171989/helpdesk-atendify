-- 1. Alterar a FK para permitir deleção em cascata (facilita testes e limpeza)
ALTER TABLE public.auditoria_recalculos_vendas 
DROP CONSTRAINT IF EXISTS auditoria_recalculos_vendas_venda_id_fkey,
ADD CONSTRAINT auditoria_recalculos_vendas_venda_id_fkey 
    FOREIGN KEY (venda_id) REFERENCES public.vendas(id) ON DELETE CASCADE;

-- 2. Simplificar a limpeza no script de teste

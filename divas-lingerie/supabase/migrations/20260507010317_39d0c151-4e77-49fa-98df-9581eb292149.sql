-- Recria a view incluindo a coluna deleted_at no final
...
CREATE OR REPLACE VIEW public.view_visitas_detalhada AS
 SELECT v.id,
    ...
    ( SELECT jsonb_agg(jsonb_build_object('id', p.id, 'descricao', p.pro_descricao, 'cor', co.cor_nome, 'tamanho', tam.tam_nome, 'categoria', cat.cat_nome, 'codigo', p.pro_codigo, 'valor_unitario', p.pro_valor_venda)) AS jsonb_agg
           FROM (((tab_produtos p
    ...
    v.deleted_at -- Adicionada ao final
   FROM (tab_visitas v
